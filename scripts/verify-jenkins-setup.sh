...
if sudo test -f /var/lib/jenkins/credentials.xml; then
    ...
    echo -e "${YELLOW}⚠️  No credentials configured yet${NC}"
fi
...
if [ $JOB_COUNT -gt 0 ]; then
    ...
    echo -e "${YELLOW}⚠️  No jobs configured yet${NC}"
fi
